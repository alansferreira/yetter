(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["YamlSetter"] = factory();
	else
		root["YamlSetter"] = factory();
})(this, () => {
...
__webpack_require__.d(__webpack_exports__, {
  castValueByType: () => (/* reexport */ castValueByType),
  parseNodeJsonPath: () => (/* reexport */ parseNodeJsonPath),
  setYamlValues: () => (/* reexport */ setYamlValues)
});
...
function setYamlValues(yamlContent, operations, options = {}) {
...
function extractDocumentSelector(path) {
...
function parseDocumentSelector(rawSelector, fullPath) {
    ...
    throw new Error(`Invalid path "${fullPath}". Document selector must be numeric like $doc[1] or filter like $doc[?(kind=='Component')].`);
}
function parseFilterCondition(condition, fullPath, parseLeftPath) {
...
function splitFilterCondition(condition, fullPath) {
    let quote = null;
    let bracketDepth = 0;
...
function parseDocumentFilterPath(pathExpression, fullPath) {
    const normalizedPath = pathExpression.startsWith('[') || pathExpression.startsWith('.')
        ? `$${pathExpression}`
        : `$.${pathExpression}`;
    try {
        ...
        throw new Error(`Invalid path "${fullPath}". Document filter path "${pathExpression}" is not valid node navigation.`);
    }
}
function parseNodeFilterPath(pathExpression, fullPath) {
    const trimmed = pathExpression.trim();
    if (trimmed === '@') {
        return [];
    }
    const normalizedPath = trimmed.startsWith('@.') || trimmed.startsWith('@[')
        ? `$${trimmed.slice(1)}`
    ...
    try {
        ...
        throw new Error(`Invalid path "${fullPath}". Filter path "${pathExpression}" is not valid node navigation.`);
    }
}
function parseFilterLiteral(rawLiteral, fullPath) {
    const quoted = /^("([^"\\]|\\.)*"|'([^'\\]|\\.)*')$/.exec(rawLiteral);
    if (quoted) {
        return rawLiteral.slice(1, -1).replace(/\\([\\'"nrt])/g, (_full, escaped) => {
            if (escaped === 'n')
                return '\n';
            if (escaped === 'r')
                return '\r';
    ...
    throw new Error(`Invalid path "${fullPath}". Filter literal "${rawLiteral}" must be string, number, boolean or null.`);
}
function normalizeNodePathFromSelector(originalPath, rest) {
    if (rest === '') {
        return '$';
    }
    ...
    throw new Error(`Invalid path "${originalPath}". After $doc[n] or $doc[?(...)], use node navigation like .name or ["name"].`);
}
function selectTargetDocuments(documents, selector, originalPath) {
    if (selector.type === 'all') {
        return documents;
    }
    if (selector.type === 'index') {
        const selected = [documents[selector.index]].filter((doc) => Boolean(doc));
...
function parseOperationPath(path, fullPath) {
    if (!path || path[0] !== '$') {
    ...
    while (cursor < path.length) {
        ...
        if (char === '.') {
            ...
            if (!identifierMatch) {
                throw new Error(`Invalid path "${path}". Dot notation only supports node names like $.node or $.node_name.`);
            }
        ...
        if (char === '[') {
            ...
            if (filterMatch) {
                ...
                if (quoted) {
                    ...
                    throw new Error(`Invalid path "${path}". Bracket notation supports numeric indexes, quoted node names or filters like [?()].`);
                }
            }
            cursor = bracketEnd + 1;
            continue;
        }
        throw new Error(`Invalid path "${path}". Only node navigation with '.' and '[index|"key"|?()]' is supported.`);
    }
    return segments;
}
function resolveConcreteTargetPaths(document, pathSegments, originalPath) {
    let candidates = [[]];
...
function matchesFilterCondition(rootValue, condition) {
...
function readValueAtPath(rootValue, path) {
...
function parseNodeJsonPath(path) {
    if (!path || path[0] !== '$') {
        throw new Error(`Invalid path "${path}". A path must start with '$'.`);
    }
    if (path === '$') {
    ...
    while (cursor < path.length) {
        ...
        if (char === '[') {
            ...
            if (quoted) {
                ...
                throw new Error(`Invalid path "${path}". Bracket notation only supports numeric indexes or quoted node names.`);
            }
            cursor = bracketEnd + 1;
            continue;
        }
...
function castValueByType(value, valueType) {
    switch (valueType) {
        case 'string':
...
function validateDocumentErrors(documents) {
...
function applyValueToDocument(document, path, value) {
    if (path.length === 0) {
        document.contents = document.createNode(value);
        return;
    }
    if (document.contents === null) {
        document.contents = document.createNode(typeof path[0] === 'number' ? [] : {});
    }
...
function ensureTrailingLineBreak(value) {
    return value.endsWith('\n') ? value : `${value}\n`;
}
function findMatchingBracket(path, openIndex) {
    if (path[openIndex] !== '[') {
